...
public class DataTypeHandler_i1 implements DataTypeHandlerInterface
{

	@Override
	public long writeInfo(RandomAccessFile targetheapfile,long heapfileptr,String st, int size) throws IOException
	{	
		int d=Integer.parseInt(st); //byte d = Byte.parseByte(st);
		targetheapfile.seek(heapfileptr);
		targetheapfile.writeByte(d);
	...
	{
		if(Byte.parseByte(field) == Byte.parseByte(value))
			return 0;
		
		else if(Byte.parseByte(field) >= Byte.parseByte(value))
			return 1;
